...
def run_one_case(use_model, **kwargs):
    ...
    if not bool(kwargs.get('headless', False)):
        fig = plt.figure()
    ...
    if bool(kwargs.get('save_to_csv', False)):
        
        # Create a DataFrame
        data = {
            "s_delta": [long_dist],
            "v_delta": [round((init_long_speed_ego-init_long_speed_c)*3.6)],
            "ego_max_dec": [ego_veh.CC_max_deceleration],
            "collision": [ego_veh.crash]
        }
        ...
        for key, value in data.items():
            filename_parts.append(f"{key}_{value[0]}")  # Append key-value pairs to the filename
        filename = "_".join(filename_parts) + ".csv"  # Join parts with underscores and add .csv extension

        # Save to CSV with the dynamically generated filename
